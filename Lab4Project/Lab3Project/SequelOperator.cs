...
namespace Lab3Project
{
    public static class SequelOperator
    {
        ...
        public static void emergencyAdd(string wht2add, string address)// Dorabotatt
        {
            string genV = "";
            int jkl = wht2add.Split(',').Length;
            for (int i = 0; i < jkl; i++)
            {
                genV += "'0'";
                if (i!= jkl-1)
            ...
            try
            {
                ...
                {
                    string theCommand = "insert into persinf("+wht2add+") Values("+genV+")";
